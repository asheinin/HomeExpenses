function addNewExpenseRM1() { // Recurrent from current month
  addNewExpense1('rm');
  return;
}  

function addNewExpenseRY1() { // Recurrent from begining of year
  addNewExpense1('ry');
  return;
}  

function addNewExpenseOT1() { // One time in current month 
  addNewExpense1('ot');
  return;
} 

function addNewExpense1(mode) {
 ...
 var response1 = myUtils.dialogQA('Enter Expense Type','Name'); 
 if (response1 == -1) throw "No New Expense"; 
 var newExpenseType = response1.getResponseText(); 
 ...
 var response = myUtils.dialogList('Create/Update Expense', data);
 if (response == -1) throw "No New Expense"; 
 var newExpenseItem = response.getResponseText(); 
...
*/
 ...
 for (var i = m; i <= n; i++) {
   ...
   var expenseItems = ss.getSheets()[i].getRange(myNumbers.expenseFirstRow,myNumbers.expenseDescrColumn, numOfRows).getValues(); 
   ...
   for (var j = 0; j <= numOfRows; j++) {
     if (expenseItems[j] == newExpenseItem) {
      ...
      if (response2 != -1) {
        ...
        Logger.log("insert payment " + j+1);

        if (ss.getSheets()[i].getRange(j+myNumbers.expenseFirstRow,myNumbers.expenseFirstPayColumn).getValue() != "") {
          ss.getSheets()[i].getRange(j+myNumbers.expenseFirstRow,myNumbers.expenseFirstPayColumn).setValue(expenseAmount);
        } else {
          if (ss.getSheets()[i].getRange(j+myNumbers.expenseFirstRow,myNumbers.expenseFirstPayColumn+1).getValue() !="") {
            ss.getSheets()[i].getRange(j+myNumbers.expenseFirstRow,myNumbers.expenseFirstPayColumn+1).setValue(expenseAmount);
          }  
        }
 ...
 if (exists) return; //throw "Expense Already Exists";
 ...
 for (var i = m; i <= n; i++) {
   ...
   for (var j = 0; j < numOfRows; j++) {  
      if ((expenseItems[j][0].length == 0) && (expenseItems[j][amountColumnArrayIndex].length == 0)) {
        ...
        ss.getSheets()[i].getRange(j+myNumbers.expenseFirstRow,myNumbers.expenseDescrColumn).setValue(newExpenseItem);
        ss.getSheets()[i].getRange(j+myNumbers.expenseFirstRow,myNumbers.expenseTypeColumn).setValue(newExpenseType);
   ...
   if (!inserted) { 
      ...
      if (j+myNumbers.expenseFirstRow < myNumbers.expenseLastRow) validateType(j+myNumbers.expenseFirstRow+1, col);  
   }   
   Logger.log(i);
