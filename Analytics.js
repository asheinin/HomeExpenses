function runAnalytics() {
    ...
    // Set Section Title at Column Q (17)
    const colQ = 17;
    const colA = 10;
    summarySheet.getRange(startRow+1, colA).setValue("Historical Spending Summary")
        .setFontWeight("normal")
        .setFontFamily("Roboto")
        .setFontSize(14)
        .setFontColor("#666666");

    const matrixRange = summarySheet.getRange(startRow + 2, colA, matrixData.length, matrixData[0].length);
    matrixRange.setValues(matrixData);

    // Formatting
    matrixRange.setWrap(true);
    summarySheet.getRange(startRow + 2, colA, 1, matrixData[0].length).setFontWeight("bold").setBackground("#f8f9fa");
    summarySheet.getRange(startRow + 3, colA + 1, matrixData.length - 1, matrixData[0].length - 1).setNumberFormat("$#,##0;;");
    ...
    // Ranges
    const rangeYear = summarySheet.getRange(startRow + 2, colA, matrixData.length, 1);
    const rangeTotal = summarySheet.getRange(startRow + 2, colA + 1, matrixData.length, 1);
    const rangeStacks = summarySheet.getRange(startRow + 2, colA + 2, matrixData.length, matrixData[0].length - 2);
    ...
    //ui.alert("Analytics refined! Total labels restored for all years and descriptions added back to the legend.");
}
