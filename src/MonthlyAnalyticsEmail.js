...
/**
 * Generates textual insights based on data.
 */
function generateInsights(current, prev, yearAgo) {
    ...
    if (yearAgo) {
        const diff = current.totalSpend - yearAgo.totalSpend;
        if (diff < 0) {
            insights.push(`Great news! You spent **${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(Math.abs(diff))} less** than the same month last year.`);
        } else {
            insights.push(`You spent **${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(diff)} more** than the same month last year.`);
        }
    }
